// Original implementation from https://github.com/jasondavies/bloomfilter.js
...
(function(exports) {
  ...
  BloomFilter.prototype.intersectionCount = function(other) {
    const unionCount = this.union(other).size();
    return (this.size() + other.size()) - unionCount;
  };

  BloomFilter.prototype.union = function(other) {
    const newFilter = new BloomFilter(this.m, this.k);
    newFilter.buckets = this.buckets.slice();
    for (var i = 0, n = newFilter.buckets.length; i < n; ++i) newFilter.buckets[i] |= other.buckets[i];
    return newFilter;
  };
